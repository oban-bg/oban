...
defmodule MyApp.ConfigProvider do
  ...
  defp parse_json(path) do
    ...
    if File.exists?(path) do
      %{"queues" => queues} = path |> File.read!() |> Jason.decode!()

      # Oban expects a keyword list of queues, JSON gives us a hash.
      # the `to_atom` is safe because this isn't user-generated.
      Enum.map(queues, fn {key, value} -> {String.to_atom(key), value} end)
    end
  end
end
```

You would then include this in your `mix.ex` file where your release is configured.
...
Then when you release your app, you just ensure that you hav a json file mounted at whatever path you specified above
that contains your desired queues:
